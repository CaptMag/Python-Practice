...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		...
		printf("%s Usage: program.exe <PID>", f);
		return EXIT_FAILURE;
	}
	...
	if (hProcess == NULL) {
		...
		printf("%s Couldn't get a handle to the process (%ld), error: %ld", f, PID, GetLastError());
		return EXIT_FAILURE;
	}
	...
	if (hThread == NULL) {
		...
		printf("%s failed to get a handle to the thread, error: %ld", f, GetLastError());
